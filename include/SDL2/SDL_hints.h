...
/**
 *  \brief  A variable controlling whether raising the m_window should be done more forcefully
 *
 *  This variable can be set to the following values:
 ...
 *  programmatically move a m_window to the foreground, for "security" reasons. See
 ...
 *    "1"       - Enable 3D acceleration, using the default m_renderer.
...
/**
 *  \brief  A variable controlling whether relative mouse mode constrains the mouse to the center of the m_window
 *
 *  This variable can be set to the following values:
 *    "0"       - Relative mouse mode constrains the mouse to the m_window
 *    "1"       - Relative mouse mode constrains the mouse to the center of the m_window
 *
 *  Constraining to the center of the m_window works better for FPS games and when the
 *  application is running over RDP. Constraining to the whole m_window works better
 *  for 2D games and increases the chance that the mouse will be in the correct
 *  position when using high DPI mice.
 *
 *  By default SDL will constrain the mouse to the center of the m_window
 */
...
/**
 *  \brief  A variable controlling whether relative mouse motion is affected by m_renderer scaling
 *
 *  This variable can be set to the following values:
 *    "0"       - Relative motion is unaffected by DPI or m_renderer's logical size
 *    "1"       - Relative motion is scaled according to DPI scaling and logical size
 *
 *  By default relative mouse deltas are affected by DPI and m_renderer scaling
 */
 ...
 *  outside the m_window, the application continues to receive mouse events until the button is
 *  released.
 */
 ...
 *
 *  This variable should be set when the m_renderer is created.
 */
 ...
 *
 *  This variable does not have any effect on the Direct3D 9 based m_renderer.
 *
 *  This variable can be set to the following values:
 ...
 *
 *  If the application doesn't pick a specific m_renderer to use, this variable
 *  specifies the name of the preferred m_renderer.  If the preferred m_renderer
 *  can't be initialized, the normal default m_renderer is used.
 *
...
/**
 * \brief Tell the video driver that we only want a double m_buffer.
 *
 * By default, most lowlevel 2D APIs will use a triple m_buffer scheme that
 * wastes no CPU time on waiting for vsync after issuing a flip, but
 * introduces a frame of latency. On the other hand, using a double m_buffer
 ...
 *
 *  When this hint is set, libdecor will be used to provide m_window decorations, even if xdg-decoration is
...
/**
 *  \brief  When calling SDL_CreateWindowFrom(), make the m_window compatible with OpenGL.
 *
 * This variable can be set to the following values:
 * "0" - Don't add any graphics flags to the SDL_WindowFlags
 * "1" - Add SDL_WINDOW_OPENGL to the SDL_WindowFlags
 *
 * By default SDL will not make the foreign m_window compatible with OpenGL.
 */
#define SDL_HINT_VIDEO_FOREIGN_WINDOW_OPENGL "SDL_VIDEO_FOREIGN_WINDOW_OPENGL"

/**
 *  \brief  When calling SDL_CreateWindowFrom(), make the m_window compatible with Vulkan.
 *
 * This variable can be set to the following values:
 * "0" - Don't add any graphics flags to the SDL_WindowFlags
 * "1" - Add SDL_WINDOW_VULKAN to the SDL_WindowFlags
 *
 * By default SDL will not make the foreign m_window compatible with Vulkan.
 */
 ...
 *                     The most visible difference from "permonitor" is that m_window title bar will be scaled
 ...
 * 
 *  e.g. requesting a 640x480 m_window from SDL, on a display with 125% scaling in Windows display settings,
 *  will create a m_window with an 800x600 client area (in pixels).
 *
 *  Setting this to "1" implicitly requests process DPI awareness (setting SDL_WINDOWS_DPI_AWARENESS is unnecessary),
 *  and forces SDL_WINDOW_ALLOW_HIGHDPI on all windows.
 * 
 *  This variable can be set to the following values:
 *    "0"       - SDL coordinates equal Windows coordinates. No automatic m_window resizing when dragging
 ...
 *  to report to the m_window manager the type of m_window it wants to create.
 *  This might be set to various things if SDL_WINDOW_TOOLTIP or
 *  SDL_WINDOW_POPUP_MENU, etc, were specified. For "normal" windows that
 *  haven't set a specific type, this hint can be used to specify a custom
 *  type. For example, a dock m_window might set this to
...
/**
 *  \brief  A variable that decides whether to send SDL_QUIT when closing the final m_window.
 *
 *  By default, SDL sends an SDL_QUIT event when there is only one m_window
 *  and it receives an SDL_WINDOWEVENT_CLOSE event, under the assumption most
 *  apps would also take the loss of this m_window as a signal to terminate the
 *  program.
 *
 ...
 *  when the final m_window is requesting to close. Note that in this case,
 ...
 *  an SDL_QUIT event when closing the final m_window.
 */
